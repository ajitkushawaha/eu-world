    ...
    <div class="page-content">
            ...
            <div class="sale_bar">
                <div class="row align-items-center">
                    <div class="col-md-4 mb-3 mb-lg-0">
                        <h5 class="m0">Cruise List</h5>
                    </div>
                    <div class="col-md-8 text-end">
 
                        <?php if (permission_access("Cruise", "add_cruise_list")) {  ?>

                        <button class="badge badge-wt" view-data-modal="true" data-controller='cruise'
                                data-href="<?php echo site_url('cruise/add-cruise-template') ?>">
                            <i class="fa-solid fa-add"></i> Add Cruise
                        </button>
                        <?php }  ?>

                        <?php if (permission_access("Cruise", "cruise_list_status")) {  ?>
                        <button class="badge badge-wt" onclick="confirm_change_status('status_change')">
                            <i class="fa-solid fa-exchange"></i> Change Status
                        </button>
                        <?php }  ?>

                        <?php if (permission_access("Cruise", "delete_cruise_list")) { ?>
                        <button class="badge badge-danger badge-wt danger disable_badge" tts-disable_badge
                                onclick="confirm_delete('formbloglist')"><i class="fa-solid fa-trash"></i> Delete
                        </button>
                        <?php } ?>
        ...
        <div class="page-content-area">
            <div class="card-body">
                <div class="row">
                    <!----------Start Search Bar ----------------->
                    <form action="<?php echo site_url('cruise/cruise-list'); ?>" method="GET"
                          class="tts-dis-content row mb-3 g-3"
                          name="cruise-list-search" onsubmit="return searchvalidateForm()">
                        <div class="col-md-3">
                            <div class="form-group form-mb-20">
                                <label>Select key to search by *</label>
                                <select name="key" class="form-control"
                                        onchange="tts_searchkey(this,'cruise-list-search')"
                ...
                <div class="table-responsive">
                    <?php $trash_uri = "cruise/remove-cruise-list"; ?>
                    <form action="<?php echo site_url($trash_uri); ?>" method="POST" tts-form="true" id="formbloglist">
                        <table class="table table-bordered table-hover ">
                            <thead class="table-active">
                            <tr>

                                <?php if (permission_access("Cruise", "delete_cruise_list") || permission_access("Cruise", "cruise_list_status")) {  ?>
                            ...
                            if (!empty($list) && is_array($list)) {
                                foreach ($list as $data) {
                                    ...
                                    <tr>
                                        ...
                                        <?php if (permission_access("Cruise", "edit_cruise_list") || permission_access("Cruise", "cruise_price_list")) { ?>
                                        <td> 
                                          
                                                <button type="button" class="actbtn dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                                                    Action
                                                </button>
                                                <ul class="dropdown-menu">
                                                    <li>
                                                        <?php if (permission_access("Cruise", "edit_cruise_list")) { ?>
                                                            <a class="dropdown-item" href="javascript:void(0);" view-data-modal="true" data-controller='customer' data-id="<?php echo dev_encode($data['id']); ?>" data-href="<?php echo site_url('/cruise/edit-cruise-template/') . dev_encode($data['id']); ?>">Edit Cruise</a>
                                                        <?php }?>
                                                    </li>
                                                    <li>
                                                        <?php if (permission_access("Cruise", "cruise_price_list")) { ?>
                                                            <a class="dropdown-item" href="<?php echo site_url('/cruise/cruise-price-list/') . dev_encode($data['id']); ?>"> Price List </a>
                                                        <?php }?>    
                                                </li>
                                                </ul>
                                ...
                                echo "<tr> <td colspan='11' class='text_center'><b>No Cruise found</b></td></tr>";
                            } ?>
                            </tbody>
                        </table>
...
<div id="status_change" class="modal fade" tabindex="-1" role="dialog"  aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <form action="<?php echo site_url('cruise/cruise-list-status-change'); ?>" method="post" tts-form="true" name="form_change_status">
