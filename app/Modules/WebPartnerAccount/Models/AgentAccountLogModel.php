...
class AgentAccountLogModel extends Model
{
    ...
    public function account_logs_detail($web_partner_id)
    {
        ...
        web_partner_account_log.created,web_partner_account_log.booking_ref_no,web_partner.company_name,web_partner.pan_name,web_partner.pan_number,
         super_admin_payment_transaction.transaction_id,
    ...
    public function debit_account_logs_without_serach($web_partner_id)
    {
        ...
        return $this->select('agent_account_log.id,agent_account_log.acc_ref_number,agent_account_log.web_partner_id,agent_account_log.acc_ref_number,agent_account_log.payment_mode,
        agent_account_log.action_type,agent_account_log.service,agent_account_log.service_log,agent_account_log.transaction_type,
        agent_account_log.credit,agent_account_log.debit,agent_account_log.balance,agent_account_log.remark,
        agent_account_log.created,agent_account_log.booking_ref_no,agent.company_name,agent.pan_holder_name,agent.pan_number,
         super_admin_payment_transaction.transaction_id, flight_booking_list.booking_ref_number as flight_booking_ref_number,
         hotel_booking_list.booking_ref_number as hotel_booking_ref_number,
         bus_booking_list.booking_ref_number as bus_booking_ref_number, holiday_booking_list.booking_ref_number as holiday_booking_ref_number,
         visa_booking_list.booking_ref_number as visa_booking_ref_number,cruise_booking_list.booking_ref_number as cruise_booking_ref_number,
         car_booking_list.booking_ref_number as car_booking_ref_number,
          ...
          flight_booking_list.api_supplier,flight_booking_list.pnr,
           CONCAT(super_admin_users.first_name," ",super_admin_users.last_name) as super_admin_staff_name,
        ')
            ->join('super_admin_users', 'super_admin_users.id = agent_account_log.user_id', 'Left')
            ->join('flight_booking_list', 'flight_booking_list.id = agent_account_log.booking_ref_no AND agent_account_log.service="flight"', 'Left')
            ->join('hotel_booking_list', 'hotel_booking_list.id = agent_account_log.booking_ref_no AND agent_account_log.service="hotel"', 'Left')
            ->join('bus_booking_list', 'bus_booking_list.id = agent_account_log.booking_ref_no AND agent_account_log.service="bus"', 'Left')
            ->join('holiday_booking_list', 'holiday_booking_list.id = agent_account_log.booking_ref_no AND agent_account_log.service="holiday"', 'Left')
            ->join('visa_booking_list', 'visa_booking_list.id = agent_account_log.booking_ref_no AND agent_account_log.service="visa"', 'Left')
            ->join('cruise_booking_list', 'cruise_booking_list.id = agent_account_log.booking_ref_no AND agent_account_log.service="cruise"', 'Left')
            ->join('car_booking_list', 'car_booking_list.id = agent_account_log.booking_ref_no AND agent_account_log.service="car"', 'Left')
            ->join("($subquery) agent", 'agent.id = agent_account_log.wl_agent_id', 'Left')
            ->join('super_admin_payment_transaction', 'super_admin_payment_transaction.id = agent_account_log.payment_transaction_id', 'Left')->where($array)
    ...
    public function credit_notes($web_partner_id)
    {
        ...
        $db = \Config\Database::connect();
        $subquery = $db->table('agent')->select('id,company_name,pan_holder_name AS pan_name,pan_number');
        $subquery = $subquery->getCompiledSelect();

        $data = $this->select("agent_account_log.id as AccountLogId,agent_account_log.web_partner_id,agent_account_log.acc_ref_number,agent_account_log.payment_mode,
        agent_account_log.action_type,agent_account_log.service,agent_account_log.service_log,agent_account_log.transaction_type,
       flight_booking_list.id as flightBookingId,flight_booking_list.booking_ref_number,flight_booking_list.fare_type,flight_booking_list.airline_remark,flight_booking_list.api_supplier,
        ...
        agent_account_log.credit,agent_account_log.debit,agent_account_log.balance,agent_account_log.remark,
        agent_account_log.created,agent_account_log.booking_ref_no,agent.company_name,agent.pan_name,agent.pan_number")->where($array)
            ->where(['agent_account_log.action_type' => 'refund', 'agent_account_log.web_partner_id' => $web_partner_id])
            ->join("($subquery) agent", 'agent.id = agent_account_log.wl_agent_id', 'Left')
            ->join('flight_booking_list', "flight_booking_list.id = agent_account_log.booking_ref_no", 'Left')
            ->join('flight_booking_travelers', "flight_booking_travelers.refund_account_id = agent_account_log.id", 'Right')
            ->orderBy("agent_account_log.id", "DESC")->paginate(40);
    ...
    function credit_notes_search($web_partner_id, $data)
    {
        ...
        if ($data['from_date'] && $data['to_date']) {
            ...
            if ($data['key'] == 'date-range') {
                ...
                flight_booking_list.id as flightBookingId,flight_booking_list.booking_ref_number,flight_booking_list.fare_type,flight_booking_list.airline_remark,flight_booking_list.api_supplier,
                    ...
                    flight_booking_travelers.*,flight_booking_travelers.id as TravelerId,
                    agent_account_log.credit,agent_account_log.debit,agent_account_log.balance,agent_account_log.remark,
                    agent_account_log.created,agent_account_log.booking_ref_no,agent.company_name,agent.pan_name,agent.pan_number")
                                ->where(['agent_account_log.action_type' => 'refund', 'agent_account_log.web_partner_id' => $web_partner_id])
                                ->join("($subquery) agent", 'agent.id = agent_account_log.wl_agent_id', 'Left')
                                ->join('flight_booking_list', "flight_booking_list.id = agent_account_log.booking_ref_no", 'Left')
                                ->join('flight_booking_travelers', "flight_booking_travelers.refund_account_id = agent_account_log.id", 'Right')
       ...
       flight_booking_list.id as flightBookingId,flight_booking_list.booking_ref_number,flight_booking_list.fare_type,flight_booking_list.airline_remark,flight_booking_list.api_supplier,
        ...
        flight_booking_travelers.*,flight_booking_travelers.id as TravelerId,
        agent_account_log.credit,agent_account_log.debit,agent_account_log.balance,agent_account_log.remark,
        agent_account_log.created,agent_account_log.booking_ref_no,web_partner.company_name,web_partner.pan_name,web_partner.pan_number")
                    ->where(['agent_account_log.action_type' => 'refund', 'agent_account_log.web_partner_id' => $web_partner_id])
                    ->join("($subquery) agent", 'agent.id = agent_account_log.wl_agent_id', 'Left')
                    ->join('flight_booking_list', "flight_booking_list.id = agent_account_log.booking_ref_no", 'Left')
                    ->join('flight_booking_travelers', "flight_booking_travelers.refund_account_id = agent_account_log.id", 'Right')
    ...
    public function credit_notes_hotel($web_partner_id)
    {
        ...
        $builder = $this->select("agent_account_log.id as AccountLogId,agent_account_log.web_partner_id,agent_account_log.acc_ref_number,agent_account_log.payment_mode,
        agent_account_log.action_type,agent_account_log.service,agent_account_log.service_log,agent_account_log.transaction_type,hotel_booking_list.country_code,
       hotel_booking_list.id as hotelBookingId,hotel_booking_list.booking_ref_number,hotel_booking_list.api_supplier,hotel_booking_list.hotel_name,hotel_booking_list.city,
        ...
        hotel_booking_list.is_domestic,hotel_booking_list.payment_status, hotel_booking_list.total_price,agent_account_log.balance,agent_account_log.remark,
        agent_account_log.credit,agent_account_log.debit,hotel_booking_list.booking_source,hotel_booking_list.agent_fare_break_up,hotel_booking_list.customer_fare_break_up,
        agent_account_log.created,agent_account_log.booking_ref_no,agent.company_name,agent.pan_name,agent.pan_number")->where($array)
            ->where(['agent_account_log.action_type' => 'refund','agent_account_log.web_partner_id' => $web_partner_id])->whereIn('hotel_booking_list.booking_source',['Wl_b2b','Wl_b2c'])
            ->join("($subquery) agent", 'agent.id = agent_account_log.wl_agent_id', 'Left')
            ->join('hotel_booking_list', "hotel_booking_list.refund_account_id = agent_account_log.id", 'Right')
            ->orderBy("agent_account_log.id", "DESC")->paginate(40);
    ...
    public function credit_notes_hotel_search($web_partner_id,$data)
    {
        ...
        $builder = $this->select("agent_account_log.id as AccountLogId,agent_account_log.web_partner_id,agent_account_log.acc_ref_number,agent_account_log.payment_mode,
        agent_account_log.action_type,agent_account_log.service,agent_account_log.service_log,agent_account_log.transaction_type,
       hotel_booking_list.id as hotelBookingId,hotel_booking_list.booking_ref_number,hotel_booking_list.api_supplier,hotel_booking_list.booking_source,
        hotel_booking_list.supplier_booking_id, hotel_booking_list.lead_passenger_name,hotel_booking_list.city,hotel_booking_list.country_code,
        ...
        hotel_booking_list.is_domestic,hotel_booking_list.payment_status,hotel_booking_list.booking_status,hotel_booking_list.customer_fare_break_up,
        hotel_booking_list.total_price,hotel_booking_list.amendment_charges,hotel_booking_list.hotel_rooms_details,
        agent_account_log.credit,agent_account_log.debit,agent_account_log.balance,agent_account_log.remark,hotel_booking_list.agent_fare_break_up,
        agent_account_log.created,agent_account_log.booking_ref_no,agent.company_name,agent.pan_name,agent.pan_number")
            ->where(['agent_account_log.action_type' => 'refund','agent_account_log.web_partner_id' => $web_partner_id])
            ->join("($subquery) agent", 'agent.id = agent_account_log.wl_agent_id', 'Left')
            ->join('hotel_booking_list', "hotel_booking_list.refund_account_id = agent_account_log.id", 'Right');
        ...
        if (isset($data['from_date']) && isset($data['to_date']) && $data['from_date'] != "" && $data['to_date'] != "") {
            $from_date = strtotime(date('Y-m-d', strtotime($data['from_date'])) . ' 00:00');
            $to_date = strtotime(date('Y-m-d', strtotime($data['to_date'])) . ' 23:59');
            $Datearray = ['agent_account_log.created >=' => $from_date, 'agent_account_log.created <=' => $to_date];
        }
    ...
    public function credit_notes_holiday($web_partner_id)
    {
        ...
        $data = $this->select("agent_account_log.id as AccountLogId,agent_account_log.web_partner_id,agent_account_log.acc_ref_number,agent_account_log.payment_mode,
        agent_account_log.action_type,agent_account_log.service,agent_account_log.service_log,agent_account_log.transaction_type,
       holiday_booking_list.id as BookingId,holiday_booking_list.booking_ref_number,holiday_booking_list.api_supplier, holiday_booking_list.payment_status,
        holiday_booking_list.package_name,holiday_booking_list.package_category,holiday_booking_list.travel_date,holiday_booking_list.booking_status,
       holiday_booking_list.confirmation_no, holiday_booking_list.total_price,holiday_booking_list.total_price,holiday_booking_list.amendment_charges,
        agent_account_log.credit,agent_account_log.debit,agent_account_log.balance,agent_account_log.remark,holiday_booking_list.booking_source,
        agent_account_log.created,agent_account_log.booking_ref_no,agent.company_name,agent.pan_name,agent.pan_number")
            ->where(['agent_account_log.action_type' => 'refund','agent_account_log.service' => 'holiday','agent_account_log.web_partner_id' => $web_partner_id])->where($array)
            ->join("($subquery) agent", 'agent.id = agent_account_log.wl_agent_id', 'Left')->whereIn('holiday_booking_list.booking_source',['Wl_b2b','Wl_b2c'])
            ->join('holiday_booking_list', "holiday_booking_list.refund_account_id = agent_account_log.id", 'Right')
            ->orderBy("agent_account_log.id", "DESC")->paginate(40);
    ...
    public function credit_notes_holiday_search($web_partner_id,$data)
    {
        ...
        agent_account_log.credit,agent_account_log.debit,agent_account_log.balance,agent_account_log.remark,holiday_booking_list.booking_source,
        agent_account_log.created,agent_account_log.booking_ref_no,agent.company_name,agent.pan_name,agent.pan_number")
            ->where(['agent_account_log.action_type' => 'refund','agent_account_log.service' => 'holiday','agent_account_log.web_partner_id' => $web_partner_id])
            ->join("($subquery) agent", 'agent.id = agent_account_log.wl_agent_id', 'Left')
            ->join('holiday_booking_list', "holiday_booking_list.refund_account_id = agent_account_log.id", 'right');
    ...
    public function credit_notes_bus($web_partner_id)
    {
        ...
        bus_booking_list.bus_name,bus_booking_list.travel_operator_pnr,bus_booking_list.ticket_no,
       ...
       bus_booking_list.total_price,
        ...
        agent_account_log.credit,agent_account_log.debit,agent_account_log.balance,agent_account_log.remark,
        agent_account_log.created,agent_account_log.booking_ref_no,agent.company_name,agent.pan_name,agent.pan_number")
            ->where(['agent_account_log.action_type' => 'refund','agent_account_log.service' => 'bus','agent_account_log.web_partner_id' => $web_partner_id])->where($array)
            ->join("($subquery) agent", 'agent.id = agent_account_log.wl_agent_id', 'Left')


            ->join('bus_booking_list', "bus_booking_list.id = agent_account_log.booking_ref_no", 'Left')->whereIn('bus_booking_list.booking_source',['Wl_b2b','Wl_b2c'])
            ->join('bus_booking_travelers', "bus_booking_travelers.refund_account_id = agent_account_log.id", 'Right')
    ...
    public function credit_notes_bus_search($web_partner_id,$data)
    {
       ...
       bus_booking_list.payment_status,bus_booking_list.booking_status, bus_booking_list.total_price,
        ...
        agent_account_log.credit,agent_account_log.debit,agent_account_log.balance,agent_account_log.remark,
        agent_account_log.created,agent_account_log.booking_ref_no,agent.company_name,agent.pan_name,agent.pan_number")
            ->where(['agent_account_log.action_type' => 'refund','agent_account_log.service' => 'bus','agent_account_log.web_partner_id' => $web_partner_id])
            ->join("($subquery) agent", 'agent.id = agent_account_log.wl_agent_id', 'Left')
            ->join('bus_booking_list', "bus_booking_list.id = agent_account_log.booking_ref_no", 'Left')
            ->join('bus_booking_travelers', "bus_booking_travelers.refund_account_id = agent_account_log.id", 'Right');
